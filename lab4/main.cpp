...
string GENERATE_CODE_FILE_PATH = "/Users/DWL/Downloads/target_code_file.txt";
...
//            cout << "error" << endl;
    ...
    // 输出三地址码的四元式表示
    cout << "************* 三地址码的四元式表示 **********" << endl;
    cout << "op\t" << "arg1\t" << "arg2\t" << "result\n";
    for (int i = 0; i < quaternion_table.size(); ++i) {
        Quaternion tmp = quaternion_table[i];
        cout << tmp.op << "\t" << tmp.arg1 << "\t" << tmp.arg2 << "\t" << tmp.res << endl;
    }
...
void generateTargetCode()
{
    ...
    // 打开要参与重定向的文件
    fstream out;
    out.open(GENERATE_CODE_FILE_PATH, ios::out);
    if (out.fail())
        return;

    // 将cout重定向到generate_code_file.txt
    cout.rdbuf(out.rdbuf());

    cout << "************* 目标代码序列 **********" << endl;

    for (i = 0; i < quaternion_table.size(); i++)
    {
        // 四元式的内容对应的字符串
        res = quaternion_table[i].res;
        arg1 = quaternion_table[i].arg1;
        arg2 = quaternion_table[i].arg2;
        switch (quaternion_table[i].op)
        {
            case ASSIGN:
                cout << "MOV " << arg1 << ", R0" << endl;
                cout << "MOV R0, " << res << endl;
                break;
            case ADD:
                cout << "MOV " << arg1 << ", R0" << endl;
                cout << "ADD " << arg2 << ", R0" << endl;
                cout << "MOV R0, " << res << endl;
                break;
            case MINUS:
                cout << "MOV " << arg1 << ", R0" << endl;
                cout << "SUB " << arg2 << ", R0" << endl;
                cout << "MOV R0, " << res << endl;
                break;
            case MULTI:
                cout << "MOV " << arg1 << ", R0" << endl;
                cout << "MUL " << arg2 << ", R0" << endl;
                cout << "MOV R0, " << res << endl;
                break;
            default:
                break;
